...
class TopGG(commands.Cog):
    ...
    async def on_dbl_vote(self, data):
        ...
        embed = discord.Embed(title="<:speed_online:1277892933491757140> [Voted](https://top.gg/bot/1144179659735572640/vote)", color=0xff0000)
        next_vote_time = datetime.datetime.now() + datetime.timedelta(hours=12)
        embed.add_field(name="<:speed_duration:1277892806760726570> Can vote again at:", value=f"<t:{int(next_vote_time.timestamp())}:R>", inline=False)
        embed.add_field(name="<:speed_settings:1277892732584726538> Total Votes:", value=str(new_count), inline=False)
        embed.add_field(name="<:speed_autorespond:1278194799656304652> Current Vote Streak:", value=str(new_streak), inline=False)
