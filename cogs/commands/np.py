...
class TimeSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        guild = interaction.client.get_guild(1340604925574516756)
        if guild:
            member = guild.get_member(self.user.id)
            if member:
                role = guild.get_role(1343881618447667230)
                if role:
                    await member.add_roles(role, reason="No prefix added")

            

        log_channel = interaction.client.get_channel(1343881937445584988)
        if log_channel:
            embed = discord.Embed(
                title="User Added to No Prefix",
                description=f"**<:speed_users:1277893558476472320> User**: [{self.user}](https://discord.com/users/{self.user.id})\n**<:speed_fun:1277893459562467389> User Mention**: {self.user.mention}\n**<:speed_info:1277823235672768629> ID**: {self.user.id}\n\n**<:speed_vip:1277892696140414989> Added By**: [{self.author.display_name}](https://discord.com/users/{self.author.id})\n<:speed_duration:1277892806760726570> **Expiry Time**: {expiry_text}\n<:speed_ArrowRed:1278191483840761909> **Timestamp**: {expiry_timestamp}\n\n<:olympus_booster:1319664386222329908> **Tier**: **{self.values[0].upper()}**",
                color=0x000000
            )
            embed.set_thumbnail(url=self.user.avatar.url if self.user.avatar else self.user.default_avatar.url)
            await log_channel.send("<@936490998362685450>",embed=embed)
            

        
        embed = discord.Embed(description=f"**Added Global No Prefix**:\n<:speed_users:1277893558476472320> **User**: **[{self.user}](https://discord.com/users/{self.user.id})**\n<:speed_fun:1277893459562467389> **User Mention**: {self.user.mention}\n<:speed_info:1277823235672768629> **User ID**: {self.user.id}\n\n__**Additional Info**__:\n<:speed_vip:1277892696140414989> **Added By**: **[{self.author.display_name}](https://discord.com/users/{self.author.id})**\n<:speed_duration:1277892806760726570> **Expiry Time:** {expiry_text}\n<:speed_ArrowRed:1278191483840761909> **Timestamp:** {expiry_timestamp}", color=0x000000)
...
class NoPrefix(commands.Cog):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if expired_users:
                ...
                for user_id in expired_users:
                    ...
                    if user:
                        log_channel = self.client.get_channel(1343881937445584988)
                        if log_channel:
                            embed_log = discord.Embed(
                                title="No Prefix Expired",
                                description=(
                                    f"**<:speed_users:1277893558476472320> User**: [{user}](https://discord.com/users/{user.id})\n"
                                    f"**<:speed_fun:1277893459562467389> User Mention**: {user.mention}\n"
                                    f"**<:speed_info:1277823235672768629> ID**: {user.id}\n\n"
                                    f"**<:speed_vip:1277892696140414989> Removed By**: **[Arbitex#7761](https://discord.com/users/1280093375630020648)**\n"
                                ),
                                color=0x000000
                            )
                            ...
                            await log_channel.send("<@936490998362685450>", embed=embed_log)
                        bot = self.client
                        guild = bot.get_guild(1340604925574516756)
                        if guild:
                        ...
                        embed = discord.Embed(
                            description=f"<:Caution:1279284925794750475> Your No Prefix status has **Expired**. You will now require the prefix to use commands.",
                            color=0x000000
                        )
                        embed.set_author(name="No Prefix Expired", icon_url=user.avatar.url if user.avatar else user.default_avatar.url)
                        
                        embed.set_footer(text="Arbitex - No Prefix, Join support to regain access.")
    ...
    async def np_add(self, ctx, user: discord.User):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if result:
                embed = discord.Embed(description=f"**{user}** is Already in No prefix list\n\n<:speed_vip:1277892696140414989> **Requested By**: [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})\n", color=0x000000)
    ...
    async def np_remove(self, ctx, user: discord.User):
        ...
        guild = ctx.bot.get_guild(1340604925574516756)
        if guild:
            member = guild.get_member(user.id)
            if member:
                role = guild.get_role(1343881618447667230)
        ...
        embed = discord.Embed(
                description=(
                    f"**<:speed_users:1277893558476472320> User**: [{user}](https://discord.com/users/{user.id})\n"
                    f"**<:speed_fun:1277893459562467389> User Mention**: {user.mention}\n"
                    f"**<:speed_info:1277823235672768629> User ID**: {user.id}\n\n"
                    f"**<:speed_vip:1277892696140414989> Removed By**: [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})\n"
                ),
            color=0x000000
        )
        ...
        log_channel = ctx.bot.get_channel(1343881937445584988)
        if log_channel:
            embed_log = discord.Embed(
        ...
        async with aiosqlite.connect('db/np.db') as db:
            ...
            if not result:
                embed = discord.Embed(
                    ...
                    description=f"**{user}** is Not in the No Prefix list\n\n"
                                f"<:speed_vip:1277892696140414989> **Requested By**: "
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"**<:speed_users:1277893558476472320> User**: [{user}](https://discord.com/users/{user.id})\n"
                    f"**<:speed_info:1277823235672768629> User ID**: {user.id}\n\n"
                    f"**<:speed_duration:1277892806760726570> Expiry**: {expire_time} ({expire_timestamp})"
                ),
                color=0x000000
            )
    ...
    async def on_member_update(self, before, after):
        if before.premium_since is None and after.premium_since is not None:
            ...
            if not await self.is_user_in_np(after.id):
                ...
                message = await log_channel.send("<@936490998362685450>", embed=embed)
    ...
    async def add_np(self, user, duration):
        ...
        embed = discord.Embed(
                            title="<:speed_giveaways:1277893483629252630> Congratulations you got 2 months No Prefix!",
                            description=f"You've been credited 2 months of global No Prefix for boosting our Partnered Servers. You can now use my commands without prefix. If you wish to remove it, please reach out [Support Server](https://discord.gg/yV7HbDYKS5).",
                            color=0x000000
                        )
        try:
        ...
        guild = self.client.get_guild(1340604925574516756)
        if guild:
            member = guild.get_member(user.id)
    ...
    async def remove_np(self, user):
        ...
        embed= discord.Embed(title="<:Caution:1279284925794750475> Global No Prefix Expired",
                        description=f"Hey {user.mention}, your global no prefix has expired!\n\n__**Reason:**__ Unboosting our partnered Server.\nIf you think this is a mistake then please reach out [Support Server](https://discord.gg/yV7HbDYKS5).",
                        color=0x000000)
