...
class Status(commands.Cog):
    ...
    async def status(self, ctx, user: discord.User = None):
        ...
        processing = await ctx.send("<a:speed_loading:1277822539732750356> Loading Status...")
        embed = discord.Embed(title=f"{user.display_name}'s Status", color=0x000000)

        status_emoji = {
            "online": "<:speed_online:1277892933491757140> Online",
            "idle": "<:speed_idle:1277892842085154898> Idle",
            "dnd": "<:speed_dnd:1277893386497687553> Do Not Disturb",
            "offline": "<:speed_offline:1277892857801211957> Offline"
        }
        ...
        if member:
            status = status_emoji.get(str(member.status), "<:speed_offline:1277892857801211957> Offline")
            ...
            try:
                ...
                embed.add_field(name="Status:", value="<:speed_offline:1277892857801211957> Offline", inline=False)
    ...
    def get_platform(self, member):
        if member.desktop_status != discord.Status.offline:
            return "<:emoji_28:1343921203999408191> Desktop"
        elif member.mobile_status != discord.Status.offline:
            return "<:SageMobile:1343916840744648736> Mobile"
        elif member.web_status != discord.Status.offline:
            return "<:icon_browser:1343916541527330830> Browser"
        return "Unknown"
