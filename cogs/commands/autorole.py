...
class BasicView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction):
        if interaction.user.id != self.ctx.author.id and interaction.user.id not in [1144179659735572640]:
...
class AutoRole(commands.Cog):
    ...
    async def _autorole(self, ctx):
        emb = discord.Embed(
            title="__AutoRole System__",
            color=0x000000
        )
        emb.add_field(
            name="Setup Commands",
            value=(
                f"`→ autorole humans add <role>`\n"
                f"`→ autorole humans remove <role>`\n"
                f"`→ autorole bots add <role>`\n"
                f"`→ autorole bots remove <role>`\n"
                f"`→ autorole config`\n"
                f"`→ autorole reset humans`\n"
                f"`→ autorole reset bots`\n"
                f"`→ autorole reset all`"
            ),
            inline=False
        )
        emb.set_footer(text="Use the above commands to manage autoroles.")
    ...
    async def _ar_config(self, ctx):
        ...
        if data:
            ...
            emb.add_field(name="<:speed_users:1277893558476472320> __Humans__", value=hums, inline=False)
            emb.add_field(name="<:speed_bot:1277823196271214624> __Bots__", value=bos, inline=False)
    ...
    async def _autorole_humans_add(self, ctx, *, role: discord.Role):
        ...
        if data:
            ...
            if role.id in humans:
                embed = discord.Embed(title="<:notify:1341230767480766546> Access Denied",
