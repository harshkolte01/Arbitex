...
class Fun(commands.Cog):
  ...
  async def mydog(self, ctx, user: discord.User):
      processing= await ctx.reply("<a:speed_loading:1277822539732750356> Processing Image...")
  ...
  async def howgay(self, ctx, *, person):
    ...
    embed.description = f'**{person} is {responses}% Gay**'
    ...
    async with aiohttp.ClientSession() as session:
      try:
        async with session.get(f"http://ip-api.com/json/{ip}") as response:
          ...
          embed.set_footer(
            text=f'Made by Arbitex Development™',
...
############################
  ...
  async def fake_ban(self, ctx, user: discord.Member):
    embed = discord.Embed(
      ...
      description=f"<:speed_tick:1277893404805697558> | {user.mention} has been successfully banned",
      color=0x000000
    )
    embed.set_footer(
