...
class AutoResponder(commands.Cog):
    ...
    async def _ar(self, ctx):
        emb = discord.Embed(
            title="__Autoresponder System__",
            color=0x000000
        )
        emb.add_field(
            name="Setup Commands",
            value=(
                f"`→ autoresponder create <name> <message>`\n"
                f"`→ autoresponder delete <name>`\n"
                f"`→ autoresponder edit <name> <massage>`\n"
                f"`→ autoresponder config`"
            ),
            inline=False
        )
        emb.set_footer(text="Use the above commands to manage autoresponder.")
    ...
    async def _create(self, ctx, name, *, message):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT COUNT(*) FROM autoresponses WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                ...
                if count >= 20:
                    return await ctx.reply(embed=discord.Embed(title="<:speed_cross:1277893420735791130> Error!",
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:speed_tick:1277893404805697558> Success",
    ...
    async def _config(self, ctx):
        ...
        if not autoresponses:
            return await ctx.reply(embed=discord.Embed(
                description=f"<:notify:1341230767480766546> | There are no autoresponders in {ctx.guild.name}",
                color=0x000000
            ))
