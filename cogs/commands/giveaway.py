...
class Giveaway(commands.Cog):
        ...
        try:
            ...
            if channel is not None:
                try:
                    ...
                    await message.edit(content="<:MekoGift:1276012607022563369> **GIVEAWAY ENDED** <:MekoGift:1276012607022563369>", embed=embed)
                    await message.reply(f"<:MekoInvestor:1255550846146580562> Congrats {winner}, you won **{giveaway[5]}!**, Hosted by <@{int(giveaway[3])}>")
                    await self.cursor.execute("DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?", (message.id, message.guild.id))
                    await self.connection.commit()
    ...
    async def gstart(self, ctx,
        ...
        embed = discord.Embed(title=f"<:MekoGift:1276012607022563369> {prize}",
                              description=f"Winner(s): **{winners}**\nReact with <:MekoInvestor:1255550846146580562> to participate!\nEnds <t:{round(ends)}:R> (<t:{round(ends)}:f>)\n\nHosted by {ctx.author.mention}", color=0x000000)
        ...
        message = await ctx.send("<:MekoGift:1276012607022563369> **GIVEAWAY** <:MekoGift:1276012607022563369>", embed=embed)
        try:
        ...
        await message.add_reaction("<:MekoInvestor:1255550846146580562>")
        await self.connection.commit()
    ...
    async def gend(self, ctx, message_id = None):
        ...
        if message_id is not None:
            ...
            if len(users) < 1:
                await ctx.send(f"<:speed_tick:1277893404805697558> Successfully Ended the giveaway in <#{int(re[6])}>")
            ...
            embed = discord.Embed(title=f"<:MekoGift:1276012607022563369> {re[5]}",
                        description=f"Ended at <t:{int(current_time)}:R>\nHosted by <@{int(re[3])}>\nWinner(s): {winner}",
                        color=0x000000
                    )
            embed.timestamp = discord.utils.utcnow()
            ...
            await message.reply(f"<:MekoFriend:1255550446290997351> Congrats {winner}, you won **{re[5]}!**, Hosted by <@{int(re[3])}>")
            await self.cursor.execute("DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?", (message.id, message.guild.id))
    ...
    async def greroll(self, ctx, message_id: typing.Optional[int] = None):
        ...
        await message.reply(f"<:MekoFriend:1255550446290997351> The new winner is "+", ".join(f"<@{i}>" for i in winners)+". Congratulations!")
        await self.connection.commit()
