...
class BlacklistWordSource(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        ...
        embed.set_footer(
            ...
            icon_url="https://cdn.discordapp.com/attachments/1341234074169118803/1341234182734610503/befunky_2025-1-0_14-5-8.png?ex=67bfcd2c&is=67be7bac&hm=b3a59a9731c75d377d4e6f7822992a5710c7f247b4def7de80cfecf631c1f917&"
        )
        return embed
...
############ FUNCTIONS ############
    ...
    @commands.group(name="blacklistword", aliases=["blword", "blw"], invoke_without_command=True)
    @blacklist_check()
    @ignore_check()
    ...
    async def bypass_add(self, ctx, target: Union[discord.Member, discord.Role]):
        ...
        if isinstance(target, discord.Member):
            ...
            if await self.is_user_bypassed(guild_id, target.id):
                embed = discord.Embed(
                    description=f"<:speed_cross:1277893420735791130> | `{target}` is already bypassed.",
                    color=discord.Color.from_rgb(0, 0, 0)
                )
                await ctx.reply(embed=embed)
                return
    ...
    async def command_error(self, ctx, error):
        if isinstance(error, commands.CommandError):
            if not isinstance(error, commands.CommandOnCooldown):
                embed = discord.Embed(
                    description="<:speed_cross:1277893420735791130> | An error occurred while processing the command. Make sure you have **Administrator** permissios.",
                    color=discord.Color.from_rgb(0, 0, 0)
                )
                await ctx.reply(embed=embed)
