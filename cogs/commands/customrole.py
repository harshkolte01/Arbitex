...
class Customrole(commands.Cog):
    ...
    async def add_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            await member.add_roles(role, reason="Arbitex Customrole | Role Added")
        else:
    ...
    async def remove_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            role = discord.Object(id=role_id)
            await member.remove_roles(role, reason="Arbitex Customrole | Role Removed")
        else:
            raise discord.Forbidden("Bot does not have permission to manage roles.")
    ...
    async def set(self, context: Context):
        emb = discord.Embed(
            title="__Customrole System__",
            color=0x000000
        )
        emb.add_field(
            name="Setup Commands",
            value=(
                f"`→ setup create <name> <role>`\n"
                f"`→ setup delete <name>`\n"
                f"`→ setup reqrole <role>`\n"
                f"`→ setup list`\n"
                f"`→ setup config`\n"
                f"`→ setup reset`\n"
                f"`→ setup staff <role>`\n"
        ...
        emb.set_footer(text="Use the above commands to manage customrole.")
    ...
    async def reset(self, context: Context) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            description=f"Deleted All Custom Role commands <:speed_tick:1277893404805697558>\n\n**Removed Roles:**\n" + "\n".join(removed_roles) if removed_roles else "No roles were previously set.",
            color=0x000000
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if result:
            ...
            if reqrole not in message.author.roles:
                await message.channel.send(embed=discord.Embed(description=f"<:Caution:1279284925794750475> You need the {reqrole.mention} role to use this command.", color=0x000000))
                return
            ...
            try:
                if role in member.roles:
                    ...
                    await message.channel.send(embed=discord.Embed(
                        title="<:speed_tick:1277893404805697558> Success",
