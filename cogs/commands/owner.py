...
async def do_removal(ctx, limit, predicate, *, before=None, after=None):
  ...
  deleted = len(deleted)
  messages = [f'<:speed_tick:1277893404805697558> | {deleted} message{" was" if deleted == 1 else "s were"} removed.']
  if deleted:
      messages.append("")
  ...
  if len(to_send) > 2000:
      await ctx.send(f"<:speed_tick:1277893404805697558> | Successfully removed {deleted} messages.", delete_after=3)
  else:
...
class Owner(commands.Cog):
    ...
    def __init__(self, client):
        ...
        self.bot_owner_ids = [936490998362685450]
    ...
    async def staff_add(self, ctx, user: discord.User):
        if user.id in self.staff:
            sonu = discord.Embed(title="<:speed_cross:1277893420735791130> Access Denied", description=f"{user} is already in the staff list.", color=0x000000)
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
            sonu2 = discord.Embed(title="<:speed_tick:1277893404805697558> Success", description=f"Added {user} to the staff list.", color=0x000000)
    ...
    async def staff_remove(self, ctx, user: discord.User):
        if user.id not in self.staff:
            sonu = discord.Embed(title="<:speed_cross:1277893420735791130> Access Denied", description=f"{user} is not in the staff list.", color=0x000000)
            await ctx.reply(embed=sonu, mention_author=False)
        else:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
                sonu2 = discord.Embed(title="<:speed_tick:1277893404805697558> Success", description=f"Removed {user} from the staff list.", color=0x000000)
            await ctx.reply(embed=sonu2, mention_author=False)
    ...
    async def staff_list(self, ctx):
        if not self.staff:
            ...
            staff_display = "\n".join(member_list)
            sonu = discord.Embed(title="<:speed_staff:1278191708563312720> Arbitex Staffs", description=f"\n{staff_display}", color=0x000000)
    ...
    async def _slist(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            description="",
            title=f"Guild List of Arbitex [{len(self.client.guilds)}]",
            color=0x000000,
            per_page=10),
            ctx=ctx)
    ...
    @commands.is_owner()
    async def getinvite(self, ctx: commands.Context, *, guild_id: Optional[int] = None):
        """Fetches active invites for a guild or creates one if none exist."""

        # Validate the guild
        guild = self.bot.get_guild(guild_id) if guild_id else ctx.guild
        if not guild:
            return await ctx.send("<:candyfloss_cross:1323487920744824926> Invalid server ID or guild not found.")

        # Check bot permissions
        if not guild.me.guild_permissions.view_audit_log:
            return await ctx.send("<:candyfloss_cross:1323487920744824926> I don't have permission to view invites.")

        try:
            invites = await guild.invites()

            if invites:
                entries = [f"[{invite.code}](https://discord.gg/{invite.code}) - {invite.uses} uses" for invite in invites]
                paginator = Paginator(
            ...
            if guild.me.guild_permissions.create_instant_invite:
                ...
                if channel:
                    ...
                    return await ctx.send(f"<:candyfloss_tick:1323480232023556127> Created new invite: {invite.url}")

                return await ctx.send("<:candyfloss_cross:1323487920744824926> No suitable channel found for invite creation.")

            return await ctx.send("<:candyfloss_cross:1323487920744824926> I don't have permission to create invites.")

        except discord.Forbidden:
            return await ctx.send("<:candyfloss_cross:1323487920744824926> I lack permissions to view or create invites.")
        except discord.NotFound:
            return await ctx.send("<:candyfloss_cross:1323487920744824926> Guild not found.")
        except discord.HTTPException as e:
            return await ctx.send(f"<:candyfloss_cross:1323487920744824926> An error occurred: `{e}`")


    @commands.command(name="Arbitex.restart", help="Restarts the client.")
    @commands.is_owner()
    async def _restart(self, ctx: Context):
        await ctx.reply("Restarting Arbitex...")
        restart_program()
    ...
    async def own_list(self, ctx):
        ...
        paginator = Paginator(source=DescriptionEmbedPaginator(
            entries=entries,
            title=f"Arbitex Owners [{len(nplist)}]",
            description="",
            per_page=10,
            color=0x000000),
                              ctx=ctx)
    ...
    async def dm(self, ctx, user: discord.User, *, message: str):
        ...
        try:
            ...
            await ctx.send(f"<:speed_tick:1277893404805697558> | Successfully Sent a DM to **{user}**")
        except discord.Forbidden:
    ...
    async def change_nickname(self, ctx, *, name: str = None):
        ...
        try:
            ...
            if name:
                await ctx.send(f"<:speed_tick:1277893404805697558> | Successfully changed nickname to **{name}**")
            else:
                await ctx.send("<:speed_tick:1277893404805697558> | Successfully removed nickname")
        ...
        if member:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<:speed_tick:1277893404805697558> | **{member.name}** has been successfully banned from {ctx.guild.name} by the Bot Owner.",
                    color=0x000000)
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:speed_cross:1277893420735791130> I do not have permission to ban **{member.name}** in this guild.",
                    color=0x000000
                )
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:speed_cross:1277893420735791130> An error occurred while banning **{member.name}**.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False, delete_after=5)
                await ctx.message.delete()
        ...
        if user:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<:speed_tick:1277893404805697558> | **{user.name}** has been successfully unbanned from {ctx.guild.name} by the Bot Owner.",
                    color=0x000000
                )
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:speed_cross:1277893420735791130> I do not have permission to unban **{user.name}** in this guild.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False)
            except discord.HTTPException:
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:speed_cross:1277893420735791130> An error occurred while unbanning **{user.name}**.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False)
        else:
...
class Badges(commands.Cog):
    ...
    async def badges(self, ctx, member: discord.Member = None):
        ...
        processing_message = await ctx.send("<a:speed_loading:1277822539732750356> Loading your profile...")
        ...
        if has_badges:
            ...
            badge_mapping = {
              "staff": "<:speed_staff:1278191708563312720> Discord Employee",
              "partner": "<:speed_sponsor:1277892049814949908> Partnered Server Owner",
              "discord_certified_moderator": "<:speed_mod:1277893508459397133> Moderator Programs Alumni",
              "hypesquad_balance": "<:emoji_35:1343890506207461428> House Balance Member",
              "hypesquad_bravery": "<:emoji_35:1343890525740072980> House Bravery Member",
              "hypesquad_brilliance": "<:emoji_37:1343890846399074335> House Brilliance Member",
              "hypesquad": "<:emoji_34:1343890476943806474> HypeSquad Events Member",
              "early_supporter": "<:olympus_early:1319664703777275916> Early Supporter",
              "bug_hunter": "<:bughunter:1343892967735169094> Bug Hunter Level 1",
              "bug_hunter_level_2": "<:bughunter2:1343893034181591081> Bug Hunter Level 2",
              "verified_bot": "<:bot:1343893443361112157> Verified Bot",
              "verified_bot_developer": "<:developer:1343894052415733803> Verified Bot Developer",
              "active_developer": "<:developer:1343894307207254140> Active Developer",
              "early_verified_bot_developer": "<:developer:1343894432218349620> Early Verified Bot Developer",
              "system": "<:speed_users:1277893558476472320> System User",
              "team_user": "<:speed_team:1277892711327731817> User is a [Team](https://discord.com/developers/docs/topics/teams)",
              "spammer": "<:speed_fun:1277893459562467389> Marked as Spammer",
              "bot_http_interactions": "<:speed_bot:1277823196271214624> Bot uses only [HTTP interactions](https://discord.com/developers/docs/interactions/receiving-and-responding#receiving-an-interaction) and is shown in the online member list."
            }
            ...
            if not member.bot:
                if omg or wtf:
                    user_badges.append("<:Rednitro:1343895125176221767> Nitro Subscriber")
                for guild in self.bot.guilds:
                    if member in guild.members:
                        if guild.premium_subscription_count > 0 and member in guild.premium_subscribers:
                            user_badges.append("<:olympus_booster:1319664386222329908> Server Booster Badge")
