...
class AutoReaction(commands.Cog):
    ...
    @commands.group(name="react", aliases=["art", "autoreact"], invoke_without_command=True)
    ...
    async def react(self, ctx):
        emb = discord.Embed(
            title="__Autoreact System__",
            color=0x000000
        )
        emb.add_field(
            name="Setup Commands",
            value=(
                f"`→ autoreact add <trigger> <emoji>`\n"
                f"`→ autoreact remove <trigger>`\n"
                f"`→ autoreact list`\n"
                f"`→ autoreact reset`"
            ),
            inline=False
        )
        emb.set_footer(text="Use the above commands to manage autoreact.")
    ...
    async def add(self, ctx, trigger: str, *, emojis: str):
        if len(trigger.split()) > 1:
            embed = discord.Embed(
                title="<:speed_cross:1277893420735791130> Invalid Trigger",
        ...
        if len(emoji_list) > 10:
            embed = discord.Embed(
                title="<:speed_cross:1277893420735791130> Too Many Emojis",
        ...
        if len(triggers) >= 10:
            embed = discord.Embed(
                title="<:notify:1341230767480766546> Trigger Limit Reached",
        ...
        if await self.trigger_exists(ctx.guild.id, trigger):
            embed = discord.Embed(
                title="<:notify:1341230767480766546> Trigger Exists",
        ...
        embed = discord.Embed(
            title="<:speed_tick:1277893404805697558> Trigger Added",
    ...
    async def remove(self, ctx, trigger: str):
        if not await self.trigger_exists(ctx.guild.id, trigger):
            embed = discord.Embed(
                title="<:speed_cross:1277893420735791130> Trigger Not Found",
        ...
        embed = discord.Embed(
            title="<:speed_tick:1277893404805697558> Trigger Removed",
    ...
    async def reset(self, ctx):
        ...
        if not triggers:
            embed = discord.Embed(
                title="<:speed_cross:1277893420735791130> No Triggers Set",
        ...
        embed = discord.Embed(
            title="<:speed_tick:1277893404805697558> All Triggers Reset",
