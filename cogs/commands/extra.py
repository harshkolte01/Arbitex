...
tick = "<:speed_tick:1277893404805697558>"
cross = "<:speed_cross:1277893420735791130>"
...
class RoleInfoView(View):
  ...
  @discord.ui.button(label='Show Permissions',  emoji="<:speed_mod:1277893508459397133>", style=discord.ButtonStyle.secondary)
  ...
  async def show_overwrites(self, interaction: discord.Interaction, button: Button):
      ...
      for target, perms in self.channel.overwrites.items():
          ...
          overwrites.append(f"**For {target.name}**\n" +
                            "\n".join(f"  * **{perm}:** {'<:speed_tick:1277893404805697558>' if value else '<:speed_cross:1277893420735791130>' if value is False else '<:Caution:1279284925794750475>'}" for perm, value in permissions.items()))
      ...
      embed.set_footer(text="<:speed_tick:1277893404805697558> = Allowed, <:speed_cross:1277893420735791130> = Denied, <:Caution:1279284925794750475> = None")
      await interaction.response.send_message(embed=embed, ephemeral=True)
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"Arbitex Uptime", color=self.color)
      embed.add_field(name="__UTC__", value=f"<:speed_info:1277823235672768629> {uptime_string}\n\n", inline=False)
      embed.add_field(name="__Online Duration__", value=f"<:speed_duration:1277892806760726570> {uptime_duration_string}", inline=False)
  ...
  async def serverinfo(self, ctx):
        ...
        embed.add_field(
            name="**__About__**",
            value=f"**Name : ** {ctx.guild.name}\n**ID :** {ctx.guild.id}\n**Owner <:speed_owner:1277823286356475955> :** {ctx.guild.owner} (<@{ctx.guild.owner_id}>)\n**Created At : ** {c_at}\n**Members :** {len(ctx.guild.members)}",
            inline=False
        )
        ...
        if ctx.guild.features:
            features = "\n".join([f"<:speed_verification:1277892897072742400> : {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}" for feature in ctx.guild.features])
            embed.add_field(
                name="**__Features__**",
        ...
        embed.add_field(
            ...
            value=f"Level: {ctx.guild.premium_tier} [<:olympus_booster:1319664386222329908> {ctx.guild.premium_subscription_count} boosts]",
            inline=False
        )
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<:speed_tick:1277893404805697558> Yes' if member.bot else '<:speed_cross:1277893420735791130> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def boosts(self, ctx):
    await ctx.send(
      embed=discord.Embed(title=f"<:olympus_booster:1319664386222329908> Boosts Count Of {ctx.guild.name}",
  ...
  async def ping(self, ctx):
      embed = discord.Embed(
          description=f"<:speed_ping:1277822753721810986> **Bot Latency:**\n\n<:speed_reddot:1278193273575440445> {round(self.bot.latency * 1000, 2)}ms",
          color=0x2a2b30  # You can adjust the color as needed
      )
  ...
  async def report(self, ctx, *, bug):
    channel = self.bot.get_channel(1343539619886796840)
    ...
    confirm_embed = discord.Embed(title="<:speed_verification:1277892897072742400> Bug Reported",
