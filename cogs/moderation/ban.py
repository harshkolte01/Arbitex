...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await self.user.send(f"<:speed_verification:1277892897072742400> You have been Unbanned from **{self.author.guild.name}** by **{self.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
...
class Ban(commands.Cog):
    ...
    async def ban(self, ctx, user: discord.User, *, reason=None):
        ...
        try:
            await user.send(f"<:Caution:1279284925794750475> You have been banned from **{ctx.guild.name}** by **{ctx.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:speed_users:1277893558476472320> Target User:** [{user}](https://discord.com/users/{user.id})\n**<:speed_fun:1277893459562467389> User Mention:** {user.mention}\n**<:speed_author:1277892998436229153> DM Sent:** {dm_status}\n**<:speed_info:1277823235672768629> Reason:** {reasonn}", color=self.color)
