...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(description=f"**<:speed_users:1277893558476472320> Target User:** [{self.user}](https://discord.com/users/{self.user.id})\n<:speed_fun:1277893459562467389> **User Mention:** {self.user.mention}\n**<:speed_ArrowRed:1278191483840761909> DM Sent:** {dm_status}\n**<:speed_info:1277823235672768629> Reason:** {reason}", color=0x000000)
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member, time: str = None, *, reason=None):
        ...
        try:
            await user.send(f"<:Caution:1279284925794750475> You have been muted in **{ctx.guild.name}** by **{ctx.author}** for {duration_text}. Reason: {reason or 'None'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:speed_users:1277893558476472320> Target User:** [{user}](https://discord.com/users/{user.id})\n"
                                          ...
                                          f"**<:speed_info:1277823235672768629> Reason:** {reason or 'None'}\n"
                                          f"**<:speed_duration:1277892806760726570> Duration:** {duration_text}",
                              color=self.color)
    ...
    async def mute_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            embed = discord.Embed(title="<:speed_cross:1277893420735791130> Access Denied", description="I don't have permission to mute members.", color=self.color)
            await ctx.send(embed=embed)
        elif isinstance(error, discord.Forbidden):
            embed = discord.Embed(title="<:speed_cross:1277893420735791130> Missing Permissions", description="I can't mute this user as they might have higher privileges (e.g., Admin).", color=self.color)
            await ctx.send(embed=embed)
            
        else:
            embed = discord.Embed(title="<:speed_cross:1277893420735791130> Unexpected Error", description=str(error), color=self.color)
            await ctx.send(embed=embed)
