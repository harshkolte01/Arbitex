...
class MuteReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await self.user.send(f"<:Caution:1279284925794750475> You have been muted in **{interaction.guild.name}** for {time_str}. Reason: {reason}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        ...
        success_embed.add_field(name="<:speed_vip:1277892696140414989> Moderator:", value=self.author.mention, inline=False)
...
class Unmute(commands.Cog):
    ...
    async def unmute(self, ctx, user: discord.Member):
        ...
        try:
            ...
            try:
                await user.send(f"<:speed_tick:1277893404805697558> You have been unmuted in **{ctx.guild.name}**.")
                dm_status = "Yes"
            except discord.Forbidden:
                dm_status = "No"
            except discord.HTTPException:
        ...
        embed = discord.Embed(
            description=f"**<:speed_users:1277893558476472320> Target User:** [{user}](https://discord.com/users/{user.id})\n**<:speed_fun:1277893459562467389> User Mention:** {user.mention}\n<:speed_ArrowRed:1278191483840761909> **DM Sent:** {dm_status}",
            color=self.color
        )
