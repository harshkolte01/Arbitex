...
class SnipeView(discord.ui.View):
    ...
    async def send_snipe_embed(self, interaction: discord.Interaction):
        ...
        embed.description = (
            f"<:speed_users:1277893558476472320> **Author:** **[{display_name}](https://discord.com/users/{uid})**\n"
            f"<:speed_info:1277823235672768629> **Author ID:** `{uid}`\n"
            f"<:speed_fun:1277893459562467389> **Author Mention:** <@{uid}>\n"
            f"<:speed_duration:1277892806760726570> **Deleted:** <t:{snipe['deleted_at']}:R>\n"
        )

        if snipe['content']:
            embed.add_field(name="<:speed_bin:1277892670727127062> **Content:**", value=snipe['content'])
        if snipe['attachments']:
            attachment_links = "\n".join([f"[{attachment['name']}]({attachment['url']})" for attachment in snipe['attachments']])
            embed.add_field(name="<:speed_media:1277893436820946977> **Attachments:**", value=attachment_links)
...
class Snipe(commands.Cog):
    ...
    async def on_message_delete(self, message):
        ...
        self.snipes[message.channel.id].insert(0, {
            ...
            'author_avatar': message.author.display_avatar.url,  # Fixed NoneType error
    ...
    async def snipe(self, ctx):
        ...
        embed.description = (
            ...
            f"<:speed_info:1277823235672768629> **Author ID:** `{uid}`\n"
            f"<:speed_fun:1277893459562467389> **Author Mention:** <@{uid}>\n"
            f"<:speed_duration:1277892806760726570> **Deleted:** <t:{first_snipe['deleted_at']}:R>\n"
        )

        if first_snipe['content']:
            embed.add_field(name="<:speed_bin:1277892670727127062> **Content:**", value=first_snipe['content'])
        if first_snipe['attachments']:
            attachment_links = "\n".join([f"[{attachment['name']}]({attachment['url']})" for attachment in first_snipe['attachments']])
            embed.add_field(name="<:speed_media:1277893436820946977> **Attachments:**", value=attachment_links)

        embed.set_footer(text=f"Total Deleted Messages: {len(channel_snipes)} | Requested by {ctx.author}", icon_url=ctx.author.display_avatar.url)
